...
### 七、 主干一致性引导下的 G/F 分布微调（联合优化）

仅凭迁移学习对 OOD 做后验校准难以显著提升 AUPR。更合理的方法是在旁路微调时引入主干预测的“一致性引导”，将 G/F 的域对齐与特性识别约束到与主干输出一致的流形附近：

1) 主干一致性能量
记主干的池化特征为 $H(x)$，旁路的上下文映射为 $\gamma(M),\beta(M)$，方向性门控为 $\alpha(x,c)$。定义一致性能量：
$$ \mathcal{E}_{\text{cons}}(x) = \big\| f_{\text{KAN}}\big( (1-\alpha)\,H + \alpha(\gamma\odot H + \beta) \big) - f_{\text{KAN}}(H) \big\|_1 $$
该能量鼓励在高置信区（基线概率高）不偏离主干输出，在低置信且门控低时也不过度抬高，形成与锚定校准相容的联合约束。

2) G/F 结构一致性
将 F 的原型对齐与 G 的去域对齐加权为与主干输出一致的方向：
$$ \mathcal{L}_{F\text{-proto}} = \mathrm{MSE}\big(z_F, \mu_c\big),\quad \mathcal{L}_{G\text{-adv}} = \mathrm{CE}\big(D_{\text{adv}}(\mathrm{GRL}(z_G)),\ \text{cell}\big) $$
对 $\mathcal{E}_{\text{cons}}$ 与上述项进行联合最小化，以旁路更新，主干冻结。

3) KL锚定一致性
对 $p_{\text{cal}}$ 与 $p_{\text{base}}$ 增加 KL 一致性项，避免概率分布发生无意义游移：
$$ \mathcal{L}_{\text{KL}} = \mathrm{KL}\big( p_{\text{cal}}\ \|\ p_{\text{base}} \big) $$

4) 总损失（旁路微调）
$$ \mathcal{L}_{\text{aux}} = w_1\,\mathcal{L}_{F\text{-proto}} + w_2\,\mathcal{L}_{G\text{-adv}} + w_3\,\mathcal{E}_{\text{cons}} + w_4\,\mathcal{L}_{\text{KL}} + \text{(orth/graph)} $$
其中 $(w_i)$ 取保守权重，保持对主干一致性的尊重而不过度抑制领域适应。该方法在训练期以主干冻结的前提下实现联合优化，推理期继续采用锚定单调校准与软回退与保守选择。

### 八、 实验诊断与改进原则（针对当前副作用）

现象（以 Colon 为例）：

- 当前版本：AUPR=0.3776, AUC=0.7279, F1=0.4646, Recall=0.6767, Precision=0.3537, Threshold=0.3300, N=4795
- 无旁路的 backbone 基线：AUPR=0.6980, AUC=0.7281, F1=0.7031, Recall=0.7873, Precision=0.6352, Threshold=0.1700, N=30336

诊断结论：

- 排序（AUC）基本未受损，但 AUPR 与 Precision 显著下滑，说明方向性校准改变了高置信区的概率刻度与阈值行为，造成 PR 面积压缩。
- 具体风险来源：
  - 锚定与温度缩放应用位置若不与“真正的基线概率”一致，会产生偏差（以 `forward()` 的输出为准，而非中途的 logit）。
  - 方向性门控 $\alpha$ 在 OOD 细胞上过强，导致对高置信正例的过度抑制（召回下降不明显但 Precision 下跌，PR 面积缩小）。
  - 软置信回退权重过弱或阈值过低，未能及时回退到基线，导致排序与刻度双重损伤。

改进原则（推理端闭式修正）：

- 基线对齐：所有锚定与阈值参考的 `p_base` 必须来自 `backbone.forward()` 的概率输出，保证与“无旁路基线”一致。
- 单调锚定：在高置信区维持与基线的单调相容，避免方向性调制翻转高置信决策；在低置信且门控低时限制上抬幅度。
- 软置信回退：用联合不置信（$u_\alpha, u_I$）的概率和实现平滑回退；在 OOD 情形下增大阈值或斜率以更保守。
- 门控下界：设置 $\alpha_{\min}\in[0.25, 0.3]$ 防止过度抑制；同时使用原型差分项削弱对“更像其他域”的样本的调制强度。
- 细胞系保守选择：每个细胞系在 `p_base/p_cal/p_mix` 中选择 AUPR 更优的作为最终记分，避免单一方案拖累总体指标。

实施要点（已在评估脚本落实）：

- 将锚定的 `hi/lo` 掩码与基线概率对齐到 `forward()` 输出的 `p_base`，避免因中途 logit 近似带来的刻度偏移。
- 将基线概率缓存为 `p_base` 并统一用于锚定与软回退；`p_cal` 仍在 logit 空间加性校准并做温度缩放后取 sigmoid。
- 继续采用 per-cell 的保守选择：按 AUPR 比较 `base/cal/mix`，选择更优变体参与该细胞系的汇总与绘图。

后续优化方向：

- 若 AUPR 仍低于基线，增大软回退的平滑系数 `k` 或提高阈值 `\tau_\alpha, \tau_I`，并减小温度缩放幅度（如 `T\in[1.0,1.1]`）。
- 用更稳健的原型估计（全量统计或指数滑动平均）替代仅暖身批次的均值，降低 OOD 原型漂移的影响。
