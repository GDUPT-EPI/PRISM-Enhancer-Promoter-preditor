...
## 工作流程
p.s. 
1. 记得在`config.py`中设置对应的`SAVEMODEL_NAME`避免覆盖之前的模型
2. 为了项目的可维护性，为避免屎山累积，单个脚本不宜超过800行，必须确保脚本的简洁性
3. 鼓励新建脚本解耦组件，鼓励优化已有逻辑而非直接拼凑新的逻辑 
4. 修改分为对训练阶段的修改和预测阶段的修改，修改好训练阶段的代码同时应该同步到推理阶段的代码
...
列出需要修改的文件和具体函数：
```
- models/PRISMModel.py: [backbone，很重要，不要随便添加内容避免冗余出错]
...
**清晰性要求**：
```python
✅ 好的实现：清晰的维度注释、清晰的调参意见
✅ 好的实现：充分解耦的组件，并放到正确地方如`models/layers/*.py`或`models/pleat/*.py`
✅ 好的实现：每个脚本不超过800行

❌ 危险的实现： 注释不清晰、无信息量的注释
❌ 危险的实现：把所有逻辑全部写到 `PRISMModel.py`
❌ 危险的实现：一千甚至数千行耦合的屎山
```
...
**模型路径是否修改为最新？**：
是否有正确修改`config.py`的`SAVEMODEL_NAME`？
