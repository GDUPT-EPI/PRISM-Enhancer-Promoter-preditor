...
---
## init: 理解项目基本情况
- 阅读 `./.kiro/steering/structure.md` 理解项目背景
- 运行 `ls docx/` 获取最新方案: `docx/记录点{i}/记录点{i}方案.md`, i = i_max
- 阅读过往的 `docx/历史索引.md` 以及 `docx/记录点{i}/记录点{i}方案.md`, i< i_max, 审查你的算法分析师同事是否又误入歧途、重蹈覆辙 
...
### Step B1: 训练日志深度分析
- 读取：`config.py`获取`{PRISM_SAVE_MODEL_DIR}/log/` 最新日志
...
#### 过拟合识别
- **训练-验证gap**：一般而言训练集效果显著优于测试集是正常的，只要没有特别严重的问题如欠拟合、模式坍塌、严重过拟合等等问题，不妨先通过质检以获得进一步的predict数据
...
### Step B2: 结果决策（二元决策，不修补）

| 状态 | 判断标准 | 决策 |
|------|----------|------|
| **通过** | 收敛良好，无明显异常 | → 继续预测流程 |
| **失败** | 任何问题（NaN/过拟合/退化/异常） | → 代码回退，重新设计 |

**核心原则**：训练有任何问题就判定失败，不尝试修复。代码会自动回退到基线，由分析师重新设计方案。
...
**如果训练失败（任何问题：NaN/过拟合/退化等）：**
STEP 1. 生成`hook` 
```bash
echo "fail" > ./hook/train_fail.txt
```
STEP 2. 生成`docx/记录点i/记录点i训练质检.md`, i=i_max

⚠️ **注意**：训练失败后代码会自动回退到基线(chat0)，不需要尝试修复！
...
- **⚠️ 必须创建hook文件**：这是触发下一步的唯一方式，不执行则工作流中断
